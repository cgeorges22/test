...
int main(int argc, char **argv) {
  ...
  altPriceIndex = false; // if false, real GDP and gdpDeflators are standard (w/based year period 0). if true then use alt index that uses nom shares as weights in gdp deflator -- 
                         // this removes hysteresis in y from idiosyncratic productivity shocks
  ...
  if(twoClasses) {
    mutScale = (double) 1.0 / (double) firmNum;
  } 
  else {
    mutScale = (double) 1.0 / (double) firmNum;
  } // 1.0 //for consumer search -- upper limit of share mutations -- should be <1 and on order of 1/firmNum -- 2/firmNum if twoClasses
  ...
  for (int i=0;i<randSeedEnd-randSeedStart+1;i++) {
    mean[i]=0;
  }
  for (int i=0;i<randSeedEnd-randSeedStart+1;i++) {
    vol[i]=0;
  ...
  for (randSeed = randSeedStart; randSeed <= randSeedEnd; randSeed ++) { //7/5/09
    ...
    initializeFirms(firms, firmNum, twoClasses, testoutput, anotation, probComplements, probInnovate, probIntegerConstrained, intPriceDiscount, wage, salary, markup); 
    // round removed 6-14-15 //markup added 5/26/16
      
    int numWithIntConstraints = 0;
    for (i=0;i<firmNum;i++) {
      if(firms[i].getIntegerConstrained()) {
    ...
    i = 0;
    while (! firms[i].getHasComplements() && i < firmNum) {
      i++;
    }
    ...
    for (i = 0; i < rounds; i++) {
      ...
      for (j = 0; j < firmNum; j++) {
        firms[j].setProfit(0.0);
      }
      
      //experiment -- change markup from 2 to 3 after round 2000 //6/10/16
      if (round==2000) {
	markup = 3.0;
	salary = 2.0;
      	for (j = 0; j < firmNum; j++) {
      ...
      if (debugging) {
        printf("update RandD investment decisions for round %d \n", round + 1);
      } //3/20/10

      if (endogInnovation && round >= startRandDInvest && round <= endRandDInvest) { 
        if (discChoice) {
          updateRandDInvestmentStatusDC(firms, firmNum, intensityOfChoice, weightRandDDC, probRandDInvestMutation, intermediateGoods, overHeadLReq, overHeadRandDReq, salary, intPriceDiscount, 
                                        medRecentRandDInvestment, avgRecentProfitAboveMedRandD, avgRecentProfitBelowMedRandD, avgRecentProfitType1, avgRecentProfitType2, testoutput, round,
                                         anotation);
        } //7/20/10 quasi discrete choice
	else { 
          updateRandDInvestmentStatus(firms, firmNum, intensityOfChoice, probRandDInvestMutation, intermediateGoods, overHeadLReq, overHeadRandDReq, salary, intPriceDiscount, testoutput, round, 
                                      anotation);
        } //3/22/10 individual social learning
      } //modified 8/30/10
      doOHandRandDAccounting(firms, firmNum, riGain,  overHeadLReq, overHeadRandDReq, salary, wageBill, salaryBill, numRandD, numRandD1, numRandD2, testoutput, round, anotation); 
      //8/31/10 //moved out of updateR&DInvestmentStatus utility
      
      //innovation
      if (debugging) {
        printf("do product and process innovation for round %d \n", round + 1);
      } //8/2/09 
      if (commonShocks) {
        updateTechEffCommonShock(firms, firmNum, testoutput, round, avTechEff, strtTech, anotation);
	...
	if(endogInnovation && round >= startProductInnovation && round <= endProductInnovation) { //3/20/10
      ...
      if (debugging) {
        printf("set prices for round %d \n", round + 1);
      } //8/2/09 
      updatePrice(firms, firmNum, /*inputFactor,*/ testoutput, anotation, intPriceDiscount); 
      if (debugging) {
        printf("order produce and sell intermediate goods for round %d \n", round + 1);
      } //8/2/09 
      ...
      if (debugging) {
        printf("produce consumption goods for round %d \n", round + 1); 
      } //8/2/09 
	...
	if (debugging) {
          printf("do consumer search for round %d \n", round + 1);
        } //8/2/09
	if(twoClasses){
	  totUtilityPL = doConsumerSearchClass1(firms, wageBill, firmNum, testoutput, round, numMutationPairs, numSearchTrials, mutScale, anotation, cesHedonics, cesHedonicsElastGoods, 
                                                cesHedonicsElastChars, complementIntConstraintFactor); 
	  totUtilityOH = doConsumerSearchClass2(firms, salaryBill, firmNum, testoutput, round, numMutationPairs, numSearchTrials, mutScale, anotation, cesHedonics, cesHedonicsElastGoods, 
                                                cesHedonicsElastChars, complementIntConstraintFactor); 
	}
	else {
          totUtility = doConsumerSearch(firms, wageBill, salaryBill, firmNum, testoutput, round, numMutationPairs, numSearchTrials, mutScale, anotation, cesHedonics, cesHedonicsElastGoods, 
                                        cesHedonicsElastChars, complementIntConstraintFactor);
        } //6/02/09 also return totUtility is new  
      }

      if (debugging) {
        printf("do final sales for round %d \n", round + 1);
      } //8/2/09
	...
	printf("total employment of production labor and OH labor are %f and %f \n", totProdLEmployment , totOHLEmployment);
      ...
      if (debugging) {
        printf("calc volatility for round %d \n", round + 1);
      } //8/2/09
      if (round == startVolatilityMeasure - volatilityMeasureFrequency) {
        lastOutput = totOutput;
      }			
	...
	output1 << round << " " << totOutput << " " << gdpDeflator << " "		
      ...
      //restarts 
      if (debugging) {
        printf("do restarts for round %d \n", round + 1);
      } //8/2/09 
      if (commonShocks) {
        restarts = doRestartsCommonShock(firms, firmNum, strtTech, wageBill, overHeadGReq, intPriceDiscount, imitation, probRandomRestart, wage, salary, markup);
      }
      else {
        if (endogN1N2) {
	  restarts = doRestartsHedonicsEndogN1N2(firms, firmNum, firmNum1, firmNum2, strtTech, wageBill, salaryBill, overHeadGReq, intPriceDiscount, imitation, probRandomRestart,
                                                 probIntegerConstrained, probComplements, wage, salary, markup, avgRecentProfitType1, avgRecentProfitType2, probSwitchMarkets, 
                                                 probRandomSwitchMarkets, ownMarketBias);
	}
      	else {
	  restarts = doRestartsHedonics(firms, firmNum, strtTech, wageBill, salaryBill, overHeadGReq, intPriceDiscount, imitation, probRandomRestart, probIntegerConstrained, probComplements, wage,
                                        salary, markup);
	}
      }
    ...
    if (qqq < firmNum) {
      printf("at end of simulation, firm %d's final demand share is %f \n", qqq,firms[qqq].getFinalDemandShare());
    }
    ...
    if (randSeed == randSeedStart) {
      testoutput << "simulation took " << secs/60 << " minutes " << "\n";
    }
