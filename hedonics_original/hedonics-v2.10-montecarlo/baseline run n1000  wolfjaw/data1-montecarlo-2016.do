...
twoway (line Yc time if time>0, clwidth(vvthin)), xlabel(#5, alternate) ylabel(#3, alternate)  /* yscale(log) */
graph save "./yc.gph",  replace 
/*twoway (line nomGDP time if time>0, clwidth(vvthin)), xlabel(#5, alternate) ylabel(#3, alternate)  /* yscale(log) */
graph save "./nomGDP.gph",  replace 
twoway (line realGDP time if time>0, clwidth(vvthin)), xlabel(#5, alternate) ylabel(#3, alternate)  yscale(log)
graph save "./realGDP.gph",  replace 
twoway (line gdpDeflator time if time>0, clwidth(vvthin)), xlabel(#5, alternate) ylabel(#3, alternate)  /* yscale(log) */
graph save "./gdpDeflator.gph",  replace 
twoway (line WageBill time if time>0, clwidth(vvthin)), xlabel(#5, alternate) ylabel(#3, alternate)  /* yscale(log) */
graph save "./wageBill.gph",  replace 
*/

twoway (line Ycsmoothed time if time>0, clwidth(vvthin)), xlabel(#5, alternate) ylabel(#3, alternate) /* yscale(log) */
graph save "./ycsmoothed.gph",  replace
/* 
twoway (line Yc time if time>0, clwidth(vvthin)) (line Ycsmoothed time if time>0, clwidth(vvthin)), ylabel(#3, alternate) 
graph save "./yc-ycsmoothed.gph", replace
twoway (line Utility time if time>0, clwidth(vvthin)) , xlabel(#5, alternate) ylabel(#3, alternate)
graph save "./utility.gph", replace
*/
twoway (line UtilityPLsmoothed time if time>0, clwidth(vvthin)) , xlabel(#5, alternate) ylabel(#3, alternate)
graph save "./utilityPLsmoothed.gph", replace
twoway (line UtilityOHsmoothed time if time>0, clwidth(vvthin)) , xlabel(#5, alternate) ylabel(#3, alternate)
...
twoway (line Utilitysmoothed1 time if time>0, clwidth(vvthin)) (line Utilitysmoothed2 time if time>0, clwidth(vvthin))
graph save "./U1andU2smoothed.gph", replace

twoway (line UtilityPL time if time>0, clwidth(vvthin)) (line UtilityOH time if time>0, clwidth(vvthin))
...
twoway (line UtilityOH time if time>0, clwidth(vvthin)) , xlabel(#5, alternate) ylabel(#3, alternate)
graph save "./utilityOH.gph", replace
twoway (line UtilityPL time if time>0, clwidth(vvthin)) , xlabel(#5, alternate) ylabel(#3, alternate)
...
twoway (line firmNum1 time if time>0, clwidth(vvthin)) (line firmNum2 time if time>0, clwidth(vvthin))
//, xlabel(#5, alternate) ylabel(#3, alternate)
...
twoway (line RandD1 time if time>0, clwidth(vvthin))(line RandD2 time if time>0, clwidth(vvthin)) 
