//  Hedonics
//      v2.11 JKR                       Oct 2016        implement parallelization
...
int main(int argc, char **argv) {
 ...
 testoutput.open(testfile.c_str(),fstream::app); 
  ...
  int *randSeedArry = new int[totalJobs];
 
  //Determine number of rand seeds each processor will recieve
  if(totalJobs < size){perJobs = 1;}
  else{perJobs = ceil((float)(totalJobs+1)/size);}

  int *jobQueueArry = new int[perJobs];

  //10/26/16 Fill randSeedArry with all randSeed that will be used
  if(rank == MASTER){
   int temp = randSeedStart;
   for (int j = 0; j <= totalJobs; j++, temp++){
     randSeedArry[j] = temp;
   }
  }
  ...
  //10/26/17 Distribute seeds to the processors
  MPI_Scatter(randSeedArry, perJobs, MPI_INT, jobQueueArry, perJobs, MPI_INT, MASTER, MPI_COMM_WORLD);

  int z = 0;
  //top loop: loop over random seeds for multiple runs
  for(randSeed = randSeedStart; randSeed <= randSeedEnd; randSeed ++) { //7/5/09
  if(jobQueueArry[z] == randSeed && z <= perJobs){  //JKR 10/26/16 make seperate processor run different seeds
    z++;    
...
//	}
  ...
  } //JKR 10/26/16 end of jobQueue[z] == randSeed if condition 
